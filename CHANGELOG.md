...
### Added
- One-shot crate-root convenience helpers: `compress(data: &[u8], level: CompressionLevel) -> Vec<u8>` and `decompress(data: &[u8], max_output_size: usize) -> Result<Vec<u8>, DecompressError>`, plus a `CompressionLevel` re-export at the crate root. `compress` wraps `encoding::compress_to_vec`; `decompress` wraps `decoding::StreamingDecoder` with a mandatory output ceiling (via `set_max_output_size`) to guard against decompression bombs. `DecompressError` is a dedicated typed error (`InvalidInput`, `OutputSizeExceeded { max_output_size }`, `Decompressor`) rather than a stringly-formatted `io::Error`, so callers can match on the failure mode instead of parsing a message; it implements `Display` unconditionally and `std::error::Error` behind the `std` feature. Both helpers are `no_std`-compatible and purely additive. The README and crate-level docs now lead the Quick start with these helpers.
